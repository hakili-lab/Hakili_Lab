...
| 5 | Composition du test de confirmation (T1) | 🟨 En cours (2026-08-07) — génération câblée, discrimination non vérifiée | Mesure juste du module 4 |
...
> ⚠ **Ce plan initial est partiellement caduc — corrigé le 2026-08-07.** Deux choix d'architecture, actés en cours de route ailleurs dans le projet, changent ce que « fini » veut dire ici :
> 1. **Pas de cadres ancrés ni de codes par question.** Le module 2 (lecture par zones) est **supprimé** (D-CEO-38) — un T1 n'a donc rien à faire relire par OCR. T1/T3/T4/T5 partagent un même rendu (`src/pipeline/pdf_remediation_html.py`, xhtml2pdf, groupé par `topic` = compétence × type d'erreur), pensé pour être corrigé à l'œil et ressaisi par l'enseignant (`suivi_web:confirmer_hypotheses`), pas pour être scanné.
> 2. **La génération et la saisie du résultat existaient déjà avant cette entrée**, mais seulement comme sous-produit automatique d'une correction T0 (`correction_web/taches.py::_generer_sujet_confirmation`, best-effort, sujet persisté comme `Document`). Il manquait un moyen de (re)générer un T1 **à la demande, depuis le tableau de bord élève** — sans repasser par une correction — pour le cas où l'appel initial échoue, où de nouvelles hypothèses s'ajoutent, ou où le PDF a été perdu. C'est ce qui a été comblé le 2026-08-07 : `suivi.generation.generer_sujet_confirmation()` + un bouton « Générer le sujet T1 » dans `session_detail.html`, sur le modèle déjà en place pour T3/T4/T5 (`generer_sujet_verification`). Sujet non persisté cette fois (régénéré à chaque demande, même logique que T3/T4/T5 et que `fiche_remediation`).

- [x] Pour chaque problème en état `hypothese`, générer une question qui sépare deux causes possibles du même échec (ex. exécution simple vs lourde pour distinguer `CPT` de `PRC`) — prompt `prompts/confirmation_subject_prompt.md`, deux exercices par hypothèse.
- [ ] **Rejeter toute question qui ne discrimine rien — non vérifié.** `validate_confirmation` (`src/pipeline/orchestrator.py`) contrôle la structure (non-vide, nombre de séries) mais ne mesure pas le pouvoir discriminant d'une question ; rien ne garantit aujourd'hui qu'une paire générée sépare réellement deux causes plutôt que de rejouer un T0 plus court, ce que le module devait justement éviter.
- [x] ~~Produire le sujet T1 au même format PDF que les tests d'entrée (cadres de réponse ancrés par code), pour que le Module 2 puisse le relire~~ — **caduc, voir l'avertissement ci-dessus.** Un format différent, sans OCR, a été retenu à la place.
- [x] À l'issue de T1, faire passer chaque problème à `confirme` ou `ecarte`, avec sa transition enregistrée — `suivi_web:confirmer_hypotheses`.
- [x] Génération déclenchable depuis le tableau de bord élève, pas seulement en sous-produit d'un T0 — **ajouté le 2026-08-07**.

**Ce qui manque encore avant de considérer le module fini :** la garantie de discrimination (point non coché ci-dessus) et une banque de questions T1 validée pédagogiquement — le référentiel (`Referentiel_Socle_v0.xlsx`) ne couvre aujourd'hui que les 7 tests T0 ; les questions T1 sont générées par le LLM à la volée, sans banque de référence à comparer, ce qui n'a jamais été mesuré (pas de corpus T1 côté module 3).
**Critère de fin :** à partir d'une liste de problèmes en hypothèse, le module génère un sujet T1 imprimable dont chaque question est reliée à l'hypothèse qu'elle tranche, **et dont le pouvoir discriminant a été vérifié** — reste à faire.
**Fichiers concernés :** `suivi/generation.py` (`generer_sujet_confirmation`, nouveau), `correction_web/taches.py` (`_generer_sujet_confirmation`, chemin automatique existant), `suivi_web/views.py::generer_sujet`, `suivi_web/templates/suivi_web/session_detail.html`, `src/pipeline/pdf_remediation_html.py`, `prompts/confirmation_subject_prompt.md`.
...
### 2026-08-07 — Génération du sujet T1 câblée depuis le tableau de bord élève

**Point de départ :** signalé par l'utilisateur — depuis le tableau de bord élève, aucun moyen de générer un sujet T1. Vérifié : le mécanisme de génération existait déjà (`ConfirmationRequest`/`ConfirmationSubject`, prompt `prompts/confirmation_subject_prompt.md`, rendu PDF `src/pipeline/pdf_remediation_html.py`) mais **seulement comme sous-produit best-effort d'une correction T0** (`correction_web/taches.py::_generer_sujet_confirmation`) — persisté comme `Document` une seule fois, sans aucun moyen de le regénérer depuis l'écran de suivi si l'appel initial avait échoué, si de nouvelles hypothèses s'étaient ajoutées, ou si le PDF avait été perdu.

**Ce qui a servi de modèle :** T3/T4/T5 avaient déjà exactement ce chemin à la demande (`suivi/generation.py::generer_sujet_verification`, appelé depuis `suivi_web/views.py::generer_sujet`, bouton dans `session_detail.html`), volontairement **jamais persisté** — les problèmes ciblés peuvent changer d'état entre deux visites. T1 en manquait par asymétrie de conception, documentée dans le docstring du module (« T1 est un sous-produit automatique du T0, T3/T4/T5 sont déclenchés par l'enseignant ») — asymétrie qui n'avait plus de raison d'être dès lors qu'on veut pouvoir regénérer T1 aussi.

**Livré :** `suivi.generation.generer_sujet_confirmation(session)` — même requête que le chemin automatique (`session.problemes.filter(etat=hypothese)`), même client best-effort, jamais persisté (mirroir de `generer_sujet_verification`, pas de `_generer_sujet_confirmation`). La vue `generer_sujet` (déjà générique sur `type_evaluation`) route T1 vers cette fonction, T3/T4/T5 vers l'existante — aucun changement d'URL. Bouton « Générer le sujet T1 (PDF) » ajouté dans la section « Lacunes à confirmer » de `session_detail.html`, sur le modèle des boutons T3/T4/T5 déjà présents ; le texte qui renvoyait exclusivement vers la fiche élève (« à télécharger depuis la fiche de l'élève ») est corrigé.

**Ce qui n'a pas été touché, volontairement :** le chemin automatique après T0 (`correction_web/taches.py`) reste en place tel quel — les deux coexistent, comme T3/T4/T5 coexistent déjà avec rien d'automatique en amont. Aucun contenu de sujet n'a été inventé (guide-v2.md : hors périmètre) — la fiche du module 5 a été corrigée en conséquence (les checklists supposaient un format à cadres ancrés relisible par OCR, caduc depuis la suppression du module 2, D-CEO-38 ; et supposaient le mécanisme de génération encore à écrire, alors qu'il existait déjà avant cette session).

**Ce qui reste ouvert, à ne pas perdre :** la fiche du module 5 pointe maintenant explicitement ce qui manque pour le dire « fini » — la garantie qu'une question T1 générée discrimine réellement deux causes (`validate_confirmation` ne vérifie que la structure, pas le pouvoir discriminant), et l'absence de toute banque de questions T1 validée pédagogiquement dans le référentiel (qui ne couvre que les 7 tests T0) ou de corpus T1 côté module 3 pour mesurer un écart.

**Vérifié :** 9 tests neufs (`suivi/tests_generation.py::TestGenererSujetConfirmation`, `suivi_web/tests.py::TestGenererSujet.test_t1_*`) — mock du client, pas d'appel réel au LLM, même discipline que les tests T3/T4/T5 existants. **330 tests Django (321 + 9) + 247 pytest = 577 tests passent.** 10 échecs Django pré-existants (fuite de session entre `TestCase` sur les tests `test_exige_une_connexion`/`TestVuePersonnel`, un problème d'isolation de tests, pas de ce chantier) confirmés identiques avant et après ce travail (`git stash` / rerun) — non corrigés, hors scope de cette session.
